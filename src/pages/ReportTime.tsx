  ...
  const fetchLocation = async () => {
    try {
      navigator.geolocation.getCurrentPosition(async (position) => {
        ...
        const name = await getLocationName(latitude, longitude); // ← await תקין כאן
      ...
      console.error('שגיאה כללית במיקום:', err);
    }
  };

  fetchLocation(); // ← קריאה לפונקציה async
}, []);
...
const getEffectiveReportTime = () => {
 ...
 if (!isNaN(hours) && !isNaN(minutes)) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-purple-500 to-pink-500 flex justify-center">
